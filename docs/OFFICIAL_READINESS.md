...
The broader internal regression probe is:
...
This probe is also public-example-derived and not leaderboard evidence. Its role is to catch obvious no-worse regressions before a new official package is sent.
...
3. Keep the broader no-worse regression probe passing as placement search changes.
